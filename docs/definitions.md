...
| email_hash | mysql: BINARY(32) / postgres: BYTEA | YES |  | Hashed email (salted/peppered). UNIQUE. | `hmac`<br/>ctx: `db.hmac.users.email_hash`<br/>kv: `email_hash_key_version` |
...
| is_locked | mysql: BOOLEAN | NO | mysql: 0 / postgres: FALSE | Lock flag (manual/automatic). |  |
...
| last_login_ip_hash | mysql: BINARY(32) / postgres: BYTEA | YES |  | Hashed last login IP. | `hmac`<br/>ctx: `db.hmac.users.last_login_ip_hash`<br/>kv: `last_login_ip_key_version` |
| last_login_ip_key_version | VARCHAR(64) | YES |  | Key version for last_login_ip_hash. | key version for: `last_login_ip_hash` |
