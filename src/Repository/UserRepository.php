...
namespace BlackCat\Database\Packages\Users\Repository;

use BlackCat\Core\Database as Database;              // e.g. BlackCat\Core\Database
use BlackCat\Database\Packages\Users\Definitions;
use BlackCat\Database\Packages\Users\Criteria;
use BlackCat\Database\Packages\Users\Dto\UserDto as Dto;
use BlackCat\Database\Packages\Users\Mapper\UserDtoMapper as RowMapper;
...
class UserRepository implements UserRepositoryInterface, RepoContract, KeysetRepoContract
{
use OrderByTools, PkTools, RepositoryHelpers;

    /** @var mixed literal token for upsert keys (array or empty). */
    private mixed $tokenUpsertKeys = [ 'email_hash' ];
     ...
     */
    protected function def(): string { return \BlackCat\Database\Packages\Users\Definitions::class; }
    ...
    public function upsertMany(array $rows): int {
        ...
        if ($helperKeys && class_exists(\BlackCat\Database\Support\BulkUpsertHelper::class)) {
          $bulk = new \BlackCat\Database\Support\BulkUpsertHelper($this->db, \BlackCat\Database\Packages\Users\Definitions::class);
          $bulk->upsertMany($rows, $helperKeys, [ 'email_hash_key_version', 'password_hash', 'password_algo', 'password_key_version', 'is_active', 'is_locked', 'failed_logins', 'must_change_password', 'last_login_at', 'last_login_ip_hash', 'last_login_ip_key_version', 'actor_role', 'deleted_at' ]);
          return count($rows);
        }
    ...
    public function getByEmailHash(string $emailHash, bool $asDto = false): array|\BlackCat\Database\Packages\Users\Dto\UserDto|null {
        return $this->getByUnique([ 'email_hash' => $emailHash ], $asDto);
    }
    public function existsByEmailHash(string $emailHash): bool {
        $where = 't.' . Ident::q($this->db, 'email_hash') . ' = :uniq_email_hash';
        return $this->exists($where, [ 'uniq_email_hash' => $emailHash ]);
    }
    /** @return int|string|null */
    public function getIdByEmailHash(string $emailHash) {
        $row = $this->getByEmailHash($emailHash, false);
