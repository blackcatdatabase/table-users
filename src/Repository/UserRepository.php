...
final class UserRepository implements RepoContract, KeysetRepoContract
{
    ...
    public function upsertMany(array $rows): int {
        ...
        if (class_exists(\BlackCat\Database\BulkUpsertRepository::class) && [ 'email_hash' ] !== []) {
            $bulk = new \BlackCat\Database\BulkUpsertRepository($this->db, \BlackCat\Database\Packages\Users\Definitions::table());
            return $bulk->upsertMany($rows, [ 'email_hash' ], [ 'email_hash_key_version', 'password_hash', 'password_algo', 'password_key_version', 'is_active', 'is_locked', 'failed_logins', 'must_change_password', 'last_login_at', 'last_login_ip_hash', 'last_login_ip_key_version', 'actor_role', 'deleted_at' ]);
        }
    ...
    */
    ...
    public function getByEmailHash(string $emailHash, bool $asDto = false): array|\BlackCat\Database\Packages\Users\Dto\UserDto|null {
        $row = $this->getByUnique([ 'email_hash' => $emailHash ]);
        if (!$asDto || !$row) return $row;
