...
use BlackCat\Database\Packages\Users\ContractRepository;

final class UserRepository implements ContractRepository {
    ...
    public function insertMany(array $rows): void {
        ...
        $sql = "INSERT INTO users (".implode(',', $cols).") VALUES ".implode(',', $allPlace);
        $this->db->execute($sql, $params);
    }
    ...
    public function upsert(array $row): void {
        ...
        $sqlBase = "INSERT INTO users (".implode(',', $cols).") VALUES (".implode(',', $place).")";
    ...
    public function updateById(int|string $id, array $row): int {
        ...
        $sql = "UPDATE users SET ".implode(',', $assign)." WHERE id = :id".$verCond;
    ...
    public function deleteById(int|string $id): int {
        ...
        if ($soft) {
            ...
            return $this->db->execute("UPDATE users SET $set WHERE id = :id", $params);
        }
    ...
    public function restoreById(int|string $id): int {
        ...
        $set = "$soft = NULL";
        if ($updAt && $updAt !== $soft) { $set .= ", $updAt = CURRENT_TIMESTAMP"; }
        return $this->db->execute("UPDATE users SET $set WHERE id = :id", [':id'=>$id]);
    }
    ...
    public function exists(string $whereSql = '1=1', array $params = []): bool {
        $sql = "SELECT 1 FROM users WHERE $whereSql LIMIT 1";
    ...
    public function paginate(Criteria $c): array {
        ...
        $total = (int)$this->db->fetchOne("SELECT COUNT(*) FROM users t $joins WHERE $where", $params);
        $items = $this->db->fetchAll("SELECT t.* FROM users t $joins WHERE $where ORDER BY $order LIMIT $limit OFFSET $offset", $params);
